  ...
  useEffect(() => {
    const loadPost = async () => {
      try {
        ...
        const postModule = await import(`../blogs/${slug}.mdx`);
  ...
  return (
    <div className="min-h-screen w-full dark:bg-[#0d1117]">
      <article className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
        <Link 
          to="/blog"
          className="inline-flex items-center gap-2 text-stone-500 hover:text-stone-700 dark:text-[#8b949e] dark:hover:text-[#c9d1d9] mb-6 transition-colors"
        >
          ← Back to blog
        </Link>
